       ...
       <div>
         {
            ...
            floor[0] && floor[0].map((item) => {
                const clickHandler = (id) =>{
                    history.push("/desck/"+id)
                }
                return (
                    access && access.includes(item.floor_num)  ?
                    //  console.log('iteem', item.floor_map_web) 
                    
                     <button  onClick={(e)=>clickHandler(item._id)}><div dangerouslySetInnerHTML={{ __html: item.floor_map_web_active}} /></button>
                    :  <input dangerouslySetInnerHTML={{ __html: item.floor_map_web}}
