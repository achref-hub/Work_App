...
export default function DesksFloor1({ floorId }){
    ...
    useEffect(() => {
        const fetch = async () => {
          const result = await fetchAccesszone(token);
          console.log(result,"jaj");
          let data =Arrayfrom        //   let data = Array.from(result, ({ Floors }) => Floors);
        //   const res = data[0].filter((e) => e._id === floorId);
        //      console.log("bb",{res});
        //   setFloors(floorId);
        };
    ...
    console.log(desks,"eeeee");
    const changeData = () => {
      socket.emit("initial_data_F1", floorId);
    }
